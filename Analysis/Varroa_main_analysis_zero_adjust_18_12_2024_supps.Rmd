...
## Varroa Supplementary Analysis

This is a repeat of the main analysis but with an offset of 1% of the given 
response values for a given treatment on a given response target for a given 
study. This will allow for the inclusion of any ratios with zero's in either 
...
The raw data set Varroa_treatment_database_2024.12.18.csv can be found in the 
...
varroa_data <- read.csv("Varroa_treatment_database_2024.12.18.csv",
                        sep = ",",
                        header = T)
...
may be several response targets such as adults, juveniles etc). 3 measures are 
dropped as the control and treatments have different signs leading
  ...
  for(z in 1:length(unique(hbi_stud$ResponseVariableTarget))){
    ...
    hbi_stud_res <- hbi_stud[hbi_stud$ResponseVariableTarget ==
     ...
     for(w in 1:length(unique(hbi_stud_res$tre_resp))){
       ...
       #This given the median value across the responses for this study
       median_hbi_tret_res <- median(hbi_tret$specificResponseMean)
...
HoneyBeeIncrease_tret_group_temp[[length(HoneyBeeIncrease_tret_group_temp)+1]]<- 
  cbind(hbi_tret[hbi_tret$Status != "control",][t,],
                   hbi_control, median_hbi_tret_res)
...
HoneyBeeIncrease_paired$logratio <- 
  log(c(HoneyBeeIncrease_paired$specificResponseMean + 
          HoneyBeeIncrease_paired$median_hbi_tret_res*0.01)/
...
is a positive outcome for bees. This loop only compares treatment values within 
studies and for the same response target (for example, within studies there
may be several response targets such as adults, juveniles etc). After zero 
adjusting there are 3 NaN values which are caused by the control and treatment 
having different signs to their values.(59 values where originally dropped)
...
is a positive outcome for bees. This loop only compares treatment values within 
studies and for the same response target (for example, within studies there
may be several response targets such as adults, juveniles etc). No values where 
removed (originally 29 infinite values were removed)
...
is a positive outcome for bees. This loop only compares treatment values within 
studies and for the same response target (for example, within studies there
may be several response targets such as adults, juveniles etc). 1 NaN value was 
removed due to change in sign between control and main value. Originally 300 
infinite values were removed.
...
when compared to synthetic chemicals, however neither are significantly 
different.

Finally, there is weak support that treatments have higher positive outcomes 
when tested in lab based setting, however, this is also not significant.
...
positive number) we create a subset for data that has some measure of dosage.
To allow for comparisons we include dosage as low medium high as either 
...
#Models without Coumaphos
#Main model

To check if the results were dependent on Coumaphos we will rerun the main
models without this chemical

```{r remove Coumaphos data }

Full_comb_nc <- Full_comb_data[Full_comb_data$broadTreatment != "Coumaphos",]

```

```{r main no Coumaphos}

mod_full_nc <- MCMCglmm(logratio ~ chem_split
                                + Context,
                                 rcov=~units,
                                 random =~StudyID_control
                                          + Continent
...
#second model acts as second chain for convergence
mod_full_nc2 <- MCMCglmm(logratio ~ chem_split
                                  + Context,
                                 rcov=~units,
                                 random =~StudyID_control
                                          + Continent
...
mod_full_nc3 <- MCMCglmm(logratio ~ chem_split
                                  + Context,
                                 rcov=~units,
                                 random =~StudyID_control
                                          + Continent
...
```{r full convergance no Coumaphos, include=FALSE}

#Check the fixed terms
main_Sol_conv_nc <- gelman.diag(mcmc.list(mod_full_nc$Sol,
                                       mod_full_nc2$Sol,
                                       mod_full_nc3$Sol))
main_Sol_conv_nc

#Check the random terms
main_VCV_conv_nc <- gelman.diag(mcmc.list(mod_full_nc$VCV,
                                       mod_full_nc2$VCV,
                                       mod_full_nc3$VCV))
main_VCV_conv_nc
...
Overall Synthetic chemicals have a significant overall positive outcome at a 
ratio of X/1 when compared to the treatment compared to control. This is 
significantly higher when compared to biological controls which only have a 
positive outcome effect at a ratio of X/1 when compared to the control.

There is some weak support that organic chemicals have less of an effect 
compared to synthetic chemicals with a ratio of X/1, however this is not 
significantly different.
...
# Model without Coumaphos effects just on bees
We can also repeat the main model with just bees included without Coumaphos.  
This is with both increases and decreases combined but with the decreases pop 
data sign flipped so now any positive value is also a positive indicator for bee 
population or health.

```{r bee all no Coumaphos}

All_bees_data <- rbind(HoneyBeeReduction_paired_red_fin, 
                       HoneyBeeIncrease_paired_fin)

All_bees_data_nc <- All_bees_data[All_bees_data$broadTreatment != "Coumaphos",]


mod_all_bees_nc <- MCMCglmm(logratio ~ chem_split
                                  + Context,
                                 rcov=~units,
                                 random =~StudyID_control 
                                          + Continent 
...
mod_all_bees_nc2 <- MCMCglmm(logratio ~ chem_split
                          + Context,
                                 rcov=~units,
                                 random =~StudyID_control 
                                          + Continent 
...
mod_all_bees_nc3 <- MCMCglmm(logratio ~ chem_split
                          + Context,
                                 rcov=~units,
                                 random =~StudyID_control 
                                          + Continent 
...
#Check the fixed terms
mod_all_bees_Sol_conv_nc <- gelman.diag(mcmc.list(mod_all_bees_nc$Sol,
                                               mod_all_bees_nc2$Sol,
                                               mod_all_bees_nc3$Sol))
mod_all_bees_Sol_conv_nc

#Check the random terms
mod_all_bees_VCV_conv_nc <- gelman.diag(mcmc.list(mod_all_bees_nc$VCV,
                                               mod_all_bees_nc2$VCV,
                                               mod_all_bees_nc3$VCV))
mod_all_bees_VCV_conv_nc


summary(mod_all_bees_nc)

```

Synthetic chemical are not found to significantly effect bee pops when compared 
to the Null, but there is some weak evidence they have perform worse than the 
control at a ratio xx/1 and that organic chemicals are less harmful compared to
syththic at xxx/1.
...
```{r Varroa both split no Coumaphos}

All_varroa_data <- rbind(VarroaReduction_paired_fin, 
                       VarroaIncrease_paired_red_fin)

All_varroa_data_nc <- All_varroa_data[All_varroa_data$broadTreatment != "Coumaphos",]


All_varroa_data_mod_nc <- MCMCglmm(logratio ~ chem_split
                                + Context,
                                 rcov=~units,
                                 random =~StudyID_control 
                                          + Continent 
...
All_varroa_data_mod_nc2 <- MCMCglmm(logratio ~ chem_split
                                 + Context,
                                 rcov=~units,
                                 random =~StudyID_control 
                                          + Continent 
...
All_varroa_data_mod_nc3 <- MCMCglmm(logratio ~ chem_split
                                 + Context,
                                 rcov=~units,
                                 random =~StudyID_control 
                                          + Continent 
...
#Check the fixed terms
All_varroa_data_mod_Sol_conv_nc <- gelman.diag(mcmc.list(All_varroa_data_mod_nc$Sol,
                                               All_varroa_data_mod_nc2$Sol,
                                               All_varroa_data_mod_nc3$Sol))
All_varroa_data_mod_Sol_conv_nc

#Check the random terms
All_varroa_data_mod_VCV_conv_nc <- gelman.diag(mcmc.list(All_varroa_data_mod_nc$VCV,
                                               All_varroa_data_mod_nc2$VCV,
                                               All_varroa_data_mod_nc3$VCV))
All_varroa_data_mod_VCV_conv_nc


summary(All_varroa_data_mod_nc)
```



#Varroa mortality models

Model Varroa on just mortality measures
...
All_varroa_data_mort <- All_varroa_data[All_varroa_data$BroadResponseVariable == "Varroa_mortality",]



All_varroa_data_mod_mort  <- MCMCglmm(logratio ~ chem_split
                                + Context,
                                 rcov=~units,
                                 random =~StudyID_control 
                                          + Continent 
...
All_varroa_data_mod_mort2 <- MCMCglmm(logratio ~ chem_split
                                 + Context,
                                 rcov=~units,
                                 random =~StudyID_control 
                                          + Continent 
...
All_varroa_data_mod_mort3 <- MCMCglmm(logratio ~ chem_split
                                 + Context,
                                 rcov=~units,
                                 random =~StudyID_control 
                                          + Continent 
...
#Check the fixed terms
All_varroa_data_mod_Sol_conv_mort <- gelman.diag(mcmc.list(All_varroa_data_mod_mort$Sol,
                                               All_varroa_data_mod_mort2$Sol,
                                               All_varroa_data_mod_mort3$Sol))
All_varroa_data_mod_Sol_conv_mort

#Check the random terms
All_varroa_data_mod_VCV_conv_mort <- gelman.diag(mcmc.list(All_varroa_data_mod_mort$VCV,
                                               All_varroa_data_mod_mort2$VCV,
                                               All_varroa_data_mod_mort3$VCV))
All_varroa_data_mod_VCV_conv_mort


summary(All_varroa_data_mod_mort)
```


#Effects of specific chemicals on mortality of Varrora

```{r chem specific model mort}

Sub_chem_mort <- Sub_chem[Sub_chem$BroadResponseVariable == "Varroa_mortality",]

mod_spec_chem_mort <- MCMCglmm(logratio ~ broadTreatment + Context,
                                 rcov=~units,
                                 random =~StudyID 
                                          + Continent 
                                          + Cont_Country,
...
mod_spec_chem_mort2 <- MCMCglmm(logratio ~ broadTreatment + Context,
                                 rcov=~units,
                                 random =~StudyID 
                                          + Continent 
                                          + Cont_Country,
...
mod_spec_chem_mort3 <- MCMCglmm(logratio ~ broadTreatment + Context,
                                 rcov=~units,
                                 random =~StudyID 
                                          + Continent 
                                          + Cont_Country,
...
#Check the fixed terms
mod_spec_chem_Sol_conv_mort <- gelman.diag(mcmc.list(mod_spec_chem_mort$Sol,
                                               mod_spec_chem_mort2$Sol,
                                               mod_spec_chem_mort3$Sol))
mod_spec_chem_Sol_conv_mort

#Check the random terms
mod_spec_chem_VCV_conv_mort <- gelman.diag(mcmc.list(mod_spec_chem_mort$VCV,
                                               mod_spec_chem_mort2$VCV,
                                               mod_spec_chem_mort3$VCV))
mod_spec_chem_VCV_conv_mort



summary(mod_spec_chem_mort)
```

Coumaphos is significantly less effective compared to Amitraz at a ratio of 
0.57/1 with all other chemicals not different. There is some evidence that Formic
acid is also less effective.
